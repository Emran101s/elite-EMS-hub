<div>
    ...
    @if ($showImport)
        <form wire:submit="import" class="card mb-4 flex flex-wrap items-end gap-3 p-4">
            <div class="flex-1">
                <label class="field-label !mb-1 !text-[0.62rem]" for="eq-import">Excel (.xlsx) or CSV — columns: <span class="font-semibold text-navy-700">name</span>, price, notes</label>
                ...
                <p class="mt-1 text-[0.6rem] text-muted">First row is treated as headers. No headers? We'll use column order: name, price, notes.</p>
                        ...
                        <tr wire:key="req-{{ $r->id }}" class="group border-b border-line last:border-0 hover:bg-page/30">
                            ...
                            <td class="px-3 py-3">
                                <div class="flex items-center justify-end gap-1 opacity-0 transition group-hover:opacity-100">
                                    <button type="button" wire:click="edit({{ $r->id }})" class="rounded-lg bg-navy-50 px-1.5 py-1 text-[0.6rem] font-bold text-navy-600 hover:bg-navy-100">✎</button>
                                    <button type="button" wire:click="delete({{ $r->id }})" wire:confirm="Delete “{{ $r->name }}” from the catalog?" class="rounded-lg bg-risk/10 px-1.5 py-1 text-[0.6rem] font-bold text-red-700 hover:bg-risk/20">✕</button>
                                </div>
                            </td>
                        </tr>
                    @endforeach
    ...
    <p class="mt-3 text-center text-[0.62rem] text-muted">{{ $items->count() }} {{ str('item')->plural($items->count()) }} in the catalog</p>

    {{-- modal --}}
    @if ($showForm)
        <div class="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-navy-900/40 p-4 pt-24 backdrop-blur-sm">
            <div class="card w-full max-w-md p-6 shadow-2xl" @click.outside="$wire.set('showForm', false)">
                <div class="mb-4 flex items-center justify-between">
                    <h3 class="pf text-base font-bold text-navy-900">{{ $editingId ? 'Edit equipment' : 'New equipment' }}</h3>
                ...
                <form wire:submit="save" class="grid gap-3.5">
                    <div>
                        <label class="field-label !mb-1 !text-[0.62rem]">Equipment name</label>
                    ...
                    <div>
                        <label class="field-label !mb-1 !text-[0.62rem]">Unit price</label>
