<div class="max-w-3xl">
    <div class="mb-5 flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
            <button type="button" wire:click="resetPackages" wire:confirm="Reset to the standard package list?" class="text-[0.62rem] font-semibold text-navy-400 hover:text-navy-700">↺ Reset to standard</button>
    ...
    <form wire:submit="save" class="grid gap-4">
        @forelse ($packages as $i => $p)
            <div wire:key="pkg-{{ $i }}" class="card p-5">
                <div class="mb-3 flex items-center gap-2">
                    <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-navy-900 text-[0.62rem] font-bold text-gold-400">{{ $i + 1 }}</span>
                    ...
                    <button type="button" wire:click="removePackage({{ $i }})" class="rounded-lg bg-risk/10 px-2 py-1.5 text-[0.7rem] font-bold text-red-700 hover:bg-risk/20">✕</button>
                </div>
                <div class="grid gap-3 sm:grid-cols-[1fr_1fr_2fr]">
                    <div>
                        <label class="field-label !mb-1 !text-[0.6rem]">Price ({{ $currency }})</label>
                        <input type="number" step="0.01" min="0" wire:model="packages.{{ $i }}.price" placeholder="0" class="input h-9 text-sm">
                    </div>
                    <div>
                        <label class="field-label !mb-1 !text-[0.6rem]">Max slots</label>
                        <input type="number" min="0" wire:model="packages.{{ $i }}.slots" placeholder="∞" class="input h-9 text-sm">
                    </div>
                    <div>
                        <label class="field-label !mb-1 !text-[0.6rem]">Benefits — one per line</label>
        ...
        @empty
            <div class="card px-6 py-12 text-center">
                <p class="text-sm font-semibold text-navy-900">No packages yet</p>
                <button type="button" wire:click="addPackage" class="btn-gold mt-4 h-10 px-5 text-xs">＋ Add your first package</button>
            </div>
        @endforelse
