...
<div class="max-w-4xl">
    <div class="mb-5">
        ...
        <h1 class="mt-1 text-lg font-bold text-ink">Transport Catalogue</h1>
    ...
    <div class="grid gap-5">
            ...
            <div class="divide-y divide-line">
                @foreach ($vehicles as $v)
                    <div wire:key="veh-{{ $v->id }}"
                         class="group flex items-center gap-3 px-5 py-2.5 {{ $v->is_active ? '' : 'bg-page' }}">
                        {{-- on/off --}}
                        <button type="button" wire:click="toggleVehicle({{ $v->id }})"
                                title="{{ $v->is_active ? 'In use — click to hide' : 'Hidden — click to use' }}"
                                class="relative h-5 w-9 shrink-0 rounded-full transition {{ $v->is_active ? 'bg-gold-500' : 'bg-line' }}">
                        ...
                        <input type="text" value="{{ $v->name }}"
                               ...
                               class="flex-1 rounded-lg border border-transparent bg-transparent px-2 py-1 text-sm font-semibold {{ $v->is_active ? 'text-ink' : 'text-muted' }} hover:border-line focus:border-gold-400/40 focus:bg-white focus:outline-none">

                        <div class="flex shrink-0 items-center gap-1.5">
                            <span class="text-[11px] font-bold uppercase tracking-wide text-muted">max</span>
                            <input type="number" min="1" value="{{ $v->capacity }}"
                                   wire:change="updateVehicle({{ $v->id }}, 'capacity', $event.target.value)"
                                   class="w-16 rounded-lg border border-line bg-white px-2 py-1 text-center text-sm font-bold text-ink focus:border-gold-400/40 focus:outline-none">
                            <span class="w-14 text-[11px] text-muted">passengers</span>
                        </div>
                        ...
                        <x-confirm title="Delete “{{ $v->name }}”?"
                                   ...
                                   class="rounded-lg px-1.5 py-1 text-[11px] font-bold text-muted opacity-0 transition hover:bg-danger/10 hover:text-danger-ink group-hover:opacity-100">✕</x-confirm>
                    </div>
                @endforeach
            </div>

            <div class="flex items-center gap-2 border-t border-line bg-page px-5 py-3">
                <input type="text" wire:model="newVehicle" wire:keydown.enter.prevent="addVehicle"
                       placeholder="New vehicle type…" class="w-full rounded-lg border border-line bg-white px-3 py-2 text-[13px] text-ink placeholder:text-muted focus:border-navy-300 focus:outline-none h-9 flex-1 text-sm">
                <input type="number" min="1" wire:model="newVehicleCapacity" wire:keydown.enter.prevent="addVehicle"
                       class="w-full rounded-lg border border-line bg-white px-3 py-2 text-[13px] text-ink placeholder:text-muted focus:border-navy-300 focus:outline-none h-9 w-20 text-center text-sm" title="Max passengers">
                <button type="button" wire:click="addVehicle" class="rounded-full bg-gold-500 px-4 py-2 text-xs font-bold text-navy-900 shadow-raise transition hover:bg-gold-400">＋ Add</button>
            </div>
            @error('newVehicle')<p class="border-t border-line bg-page px-5 pb-3 text-xs text-danger-ink">{{ $message }}</p>@enderror
        </div>
        ...
        <div class="rounded-lg border border-line bg-white shadow-raise overflow-hidden">
            ...
            <div class="divide-y divide-line">
                @foreach ($services as $s)
                    <div wire:key="svc-{{ $s->id }}"
                         class="group flex items-center gap-3 px-5 py-2.5 {{ $s->is_active ? '' : 'bg-page' }}">
                        <button type="button" wire:click="toggleService({{ $s->id }})"
                                title="{{ $s->is_active ? 'In use — click to hide' : 'Hidden — click to use' }}"
                                class="relative h-5 w-9 shrink-0 rounded-full transition {{ $s->is_active ? 'bg-gold-500' : 'bg-line' }}">
                        ...
                        <input type="text" value="{{ $s->name }}"
                               ...
                               class="flex-1 rounded-lg border border-transparent bg-transparent px-2 py-1 text-sm font-semibold {{ $s->is_active ? 'text-ink' : 'text-muted' }} hover:border-line focus:border-gold-400/40 focus:bg-white focus:outline-none">
            ...
            <div class="flex items-center gap-2 border-t border-line bg-page px-5 py-3">
                <input type="text" wire:model="newService" wire:keydown.enter.prevent="addService"
                       placeholder="New service type…" class="w-full rounded-lg border border-line bg-white px-3 py-2 text-[13px] text-ink placeholder:text-muted focus:border-navy-300 focus:outline-none h-9 flex-1 text-sm">
                <button type="button" wire:click="addService" class="rounded-full bg-gold-500 px-4 py-2 text-xs font-bold text-navy-900 shadow-raise transition hover:bg-gold-400">＋ Add</button>
            </div>
            @error('newService')<p class="border-t border-line bg-page px-5 pb-3 text-xs text-danger-ink">{{ $message }}</p>@enderror
        </div>
        ...
        <div class="rounded-lg border border-line bg-white shadow-raise overflow-hidden">
                ...
                @forelse ($drivers as $d)
                    <div wire:key="drv-{{ $d->id }}" class="group flex flex-wrap items-center gap-2 px-5 py-2.5">
                        <button type="button" wire:click="toggleDriver({{ $d->id }})"
                                ...
                                class="relative h-5 w-9 shrink-0 rounded-full transition {{ $d->is_active ? 'bg-gold-500' : 'bg-line' }}">
                        ...
                        <input type="text" value="{{ $d->name }}"
                               ...
                               class="min-w-[9rem] flex-1 rounded-lg border border-transparent bg-transparent px-2 py-1 text-sm font-semibold {{ $d->is_active ? 'text-ink' : 'text-muted' }} hover:border-line focus:border-gold-400/40 focus:bg-white focus:outline-none">

                        <input type="text" value="{{ $d->phone }}" placeholder="phone"
                               wire:change="updateDriver({{ $d->id }}, 'phone', $event.target.value)"
                               class="w-36 rounded-lg border border-transparent bg-transparent px-2 py-1 text-xs text-ink hover:border-line focus:border-gold-400/40 focus:bg-white focus:outline-none">

                        <select wire:change="setDriverSupplier({{ $d->id }}, $event.target.value)"
                                class="h-8 w-auto max-w-[10rem] rounded-lg border border-line px-1.5 text-[11px] font-semibold text-ink">
            ...
            <div class="flex flex-wrap items-center gap-2 border-t border-line bg-page px-5 py-3">
                <input type="text" wire:model="newDriver" wire:keydown.enter.prevent="addDriver"
                       placeholder="Driver name…" class="w-full rounded-lg border border-line bg-white px-3 py-2 text-[13px] text-ink placeholder:text-muted focus:border-navy-300 focus:outline-none h-9 min-w-[10rem] flex-1 text-sm">
                <input type="text" wire:model="newDriverPhone" wire:keydown.enter.prevent="addDriver"
                       placeholder="Phone" class="w-full rounded-lg border border-line bg-white px-3 py-2 text-[13px] text-ink placeholder:text-muted focus:border-navy-300 focus:outline-none h-9 w-40 text-sm">
                <select wire:model="newDriverSupplier" class="w-full rounded-lg border border-line bg-white px-3 py-2 text-[13px] text-ink focus:border-navy-300 focus:outline-none h-9 w-auto text-sm">
                    <option value="">— supplier —</option>
                    @foreach ($suppliers as $s)<option value="{{ $s->id }}">{{ $s->name }}</option>@endforeach
                </select>
                <button type="button" wire:click="addDriver" class="rounded-full bg-gold-500 px-4 py-2 text-xs font-bold text-navy-900 shadow-raise transition hover:bg-gold-400">＋ Add</button>
            </div>
        </div>
        ...
        <div class="rounded-lg border border-line bg-white shadow-raise overflow-hidden">
                ...
                @forelse ($fleet as $v)
                    <div wire:key="flt-{{ $v->id }}" class="group flex flex-wrap items-center gap-2 px-5 py-2.5">
                        ...
                        <input type="text" value="{{ $v->plate_no }}" placeholder="plate"
                               ...
                               class="w-28 rounded-lg border border-transparent bg-transparent px-2 py-1 text-sm font-bold {{ $v->is_active ? 'text-ink' : 'text-muted' }} hover:border-line focus:border-gold-400/40 focus:bg-white focus:outline-none">

                        <input type="text" value="{{ $v->model }}" placeholder="model"
                               wire:change="updateFleetVehicle({{ $v->id }}, 'model', $event.target.value)"
                               class="min-w-[8rem] flex-1 rounded-lg border border-transparent bg-transparent px-2 py-1 text-xs text-ink hover:border-line focus:border-gold-400/40 focus:bg-white focus:outline-none">
            ...
            <div class="flex flex-wrap items-center gap-2 border-t border-line bg-page px-5 py-3">
                <input type="text" wire:model="newPlate" wire:keydown.enter.prevent="addFleetVehicle"
                       placeholder="Plate…" class="w-full rounded-lg border border-line bg-white px-3 py-2 text-[13px] text-ink placeholder:text-muted focus:border-navy-300 focus:outline-none h-9 w-32 text-sm">
                <input type="text" wire:model="newModel" wire:keydown.enter.prevent="addFleetVehicle"
                       placeholder="Model, e.g. Mercedes V-Class" class="w-full rounded-lg border border-line bg-white px-3 py-2 text-[13px] text-ink placeholder:text-muted focus:border-navy-300 focus:outline-none h-9 min-w-[10rem] flex-1 text-sm">
                <select wire:model="newFleetType" class="w-full rounded-lg border border-line bg-white px-3 py-2 text-[13px] text-ink focus:border-navy-300 focus:outline-none h-9 w-auto text-sm">
                    <option value="">— type —</option>
                    @foreach ($vehicles as $t)<option value="{{ $t->id }}">{{ $t->label() }}</option>@endforeach
                </select>
                <button type="button" wire:click="addFleetVehicle" class="rounded-full bg-gold-500 px-4 py-2 text-xs font-bold text-navy-900 shadow-raise transition hover:bg-gold-400">＋ Add</button>
            </div>
            @error('newPlate')<p class="border-t border-line bg-page px-5 pb-3 text-xs text-danger-ink">{{ $message }}</p>@enderror
        </div>
    </div>
</div>
