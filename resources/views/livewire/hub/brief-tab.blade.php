@php $serif = 'font-family: \'Playfair Display\', Georgia, \'Times New Roman\', serif;'; @endphp
<div class="-mx-1">
    {{-- ══ Command bar ══ --}}
    <div class="sticky top-16 z-20 mb-5 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-navy-800/10 bg-navy-950 px-5 py-3.5 shadow-[0_18px_50px_-20px_rgba(11,31,58,0.6)]">
        ...
        <div class="relative flex items-center gap-3">
            ...
            <div>
                <p class="text-[0.55rem] font-bold uppercase tracking-[0.32em] text-gold-400">Event Dossier</p>
            ...
            <select wire:change="switchTemplate($event.target.value)"
                    ...
                    class="ml-3 h-8 rounded-xl border border-white/15 bg-white/5 px-2 text-[0.68rem] font-semibold text-white focus:outline-none">
        ...
        <div class="relative flex flex-wrap items-center gap-2">
            <span class="mr-1 flex items-center gap-1.5 text-[0.6rem] font-medium text-white/50">
            ...
            <div class="flex items-center gap-1.5 rounded-xl border border-white/15 bg-white/5 px-2.5 py-1.5">
                <span class="text-[0.55rem] font-bold uppercase tracking-wider text-white/40">Ver</span>
            ...
            <button type="button" wire:click="toggleApproved"
                    class="flex h-9 items-center gap-1.5 rounded-xl px-3 text-[0.7rem] font-bold transition {{ $status === 'approved' ? 'bg-emerald-400/15 text-emerald-300 ring-1 ring-emerald-400/30' : 'bg-white/5 text-white/60 ring-1 ring-white/15 hover:text-white' }}">
                {{ $status === 'approved' ? '✓ Approved' : 'Approve' }}
            </button>
            <button type="button" wire:click="generatePlan"
                    wire:confirm="Generate the working plan from this brief? Tasks, milestones, budget categories, risks, sponsor packages and approval gates will be created. Existing items are never overwritten."
                    @disabled($status !== 'approved')
                    title="{{ $status === 'approved' ? 'Generate plan, budget, risks, sponsors & approvals' : 'Approve the brief first' }}"
                    class="flex h-9 items-center gap-1.5 rounded-xl px-3 text-[0.7rem] font-bold transition {{ $status === 'approved' ? 'bg-white/10 text-white ring-1 ring-white/20 hover:bg-white/15' : 'cursor-not-allowed bg-white/5 text-white/25 ring-1 ring-white/10' }}">
            ...
            <a href="{{ route('events.brief.pdf', $event) }}" target="_blank"
               class="flex h-9 items-center gap-1.5 rounded-xl bg-gradient-to-r from-gold-400 to-gold-600 px-4 text-[0.72rem] font-bold text-navy-950 shadow transition hover:brightness-105">
        ...
        <div class="mb-5 overflow-hidden rounded-2xl border border-emerald-200 bg-gradient-to-r from-emerald-50 to-white px-5 py-4">
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="flex flex-wrap gap-1.5">
                    @foreach (['tasks' => 'Tasks', 'milestones' => 'Milestones', 'budget' => 'Budget', 'risks' => 'Risks', 'sponsors' => 'Sponsors', 'approvals' => 'Approvals'] as $k => $lbl)
                        <span class="rounded-full bg-white px-2.5 py-1 text-[0.6rem] font-bold text-navy-600 ring-1 ring-line">
    ...
    <div class="flex gap-6">
        ...
        <aside class="hidden w-48 shrink-0 lg:block">
            <nav class="sticky top-32 space-y-0.5">
                <p class="mb-2 px-2 text-[0.55rem] font-bold uppercase tracking-[0.2em] text-navy-300">Contents</p>
                @foreach ($sections as $key => [$num, $title, $type])
                    <a href="#sec-{{ $key }}" class="group flex items-center gap-2.5 rounded-lg px-2 py-1.5 text-[0.72rem] text-navy-500 transition hover:bg-gold-50/60 hover:text-navy-900">
                        <span class="w-4 text-right text-[0.6rem] font-bold text-gold-500/70">{{ $num }}</span>
        ...
        <main class="min-w-0 flex-1">
            <div class="mx-auto max-w-3xl overflow-hidden rounded-[1.75rem] bg-white shadow-[0_40px_90px_-40px_rgba(11,31,58,0.4)] ring-1 ring-line/70">
                {{-- Dossier hero --}}
                <div class="relative overflow-hidden bg-gradient-to-br from-navy-900 via-navy-900 to-[#071528] px-8 pb-8 pt-10 text-white sm:px-12">
                    ...
                    <div class="relative">
                        <p class="pt-3 text-[0.6rem] font-bold uppercase tracking-[0.36em] text-gold-400">◆ Elite Business Hub · Event Dossier</p>
                        ...
                        <div class="mt-6 grid grid-cols-2 gap-x-8 gap-y-4 border-t border-white/10 pt-5 sm:grid-cols-3">
                            @foreach ([['Prepared For','prepared_for'],['Prepared By','prepared_by'],['Confidentiality','confidentiality']] as [$lbl,$mk])
                                <div>
                                    <p class="text-[0.5rem] font-bold uppercase tracking-[0.2em] text-white/40">{{ $lbl }}</p>
                                    <input type="text" wire:model.blur="data.meta.{{ $mk }}" class="mt-1 w-full border-0 border-b border-transparent bg-transparent p-0 pb-0.5 text-[0.82rem] font-medium text-white transition placeholder:text-white/30 focus:border-gold-400/60 focus:outline-none focus:ring-0">
                                </div>
                            @endforeach
                        </div>
                    </div>
                ...
                <div class="border-b border-line bg-gold-50/40 px-8 py-3.5 sm:px-12">
                    <div class="flex items-start gap-2.5">
                        ...
                        <textarea wire:model.blur="data.meta.how_to" rows="2" class="w-full resize-none border-0 bg-transparent p-0 text-[0.72rem] italic leading-relaxed text-navy-500 focus:outline-none focus:ring-0" placeholder="How to use this document…"></textarea>
                    </div>
                </div>
                        ...
                        <section id="sec-{{ $key }}" wire:key="sec-{{ $key }}" class="scroll-mt-32">
                            ...
                            <div class="mb-5 flex items-end justify-between gap-3 border-b border-line pb-2.5">
                                <div class="flex items-baseline gap-3">
                                    <span class="text-2xl font-bold leading-none text-gold-400/40" style="{{ $serif }}">{{ str_pad($num, 2, '0', STR_PAD_LEFT) }}</span>
                                    <h3 class="pf text-lg font-bold text-navy-900" style="{{ $serif }}">{{ $title }}</h3>
                                </div>
                                @if (in_array($type, ['bullets', 'kpi', 'twocol', 'approval'], true))
                                    <button type="button" wire:click="addRow('{{ $key }}')" class="shrink-0 rounded-full border border-line px-2.5 py-1 text-[0.6rem] font-bold uppercase tracking-wide text-navy-400 transition hover:border-gold-300 hover:text-gold-600">＋ Add</button>
                                @endif
                            </div>
                ...
                <div class="border-t border-line bg-navy-950 px-8 py-4 text-center sm:px-12">
                    <p class="text-[0.55rem] font-medium uppercase tracking-[0.3em] text-white/40">{{ $data['meta']['confidentiality'] ?? 'Confidential' }} · Elite Business Hub · v{{ $version }}</p>
                </div>
            </div>
        </main>
    </div>
