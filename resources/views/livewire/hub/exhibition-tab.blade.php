                ...
                <tbody>
                    @foreach ($exhibitors as $x)
                        ...
                        <tr wire:key="ex-{{ $x->id }}" class="group/ex border-b border-line last:border-0 hover:bg-page/30 {{ $this->isSelected($x->id) ? 'bg-navy-50/60' : '' }}">
                            <td class="pl-4"><button type="button" wire:click="toggleSelect({{ $x->id }})" class="flex h-4 w-4 items-center justify-center rounded border text-eyebrow {{ $this->isSelected($x->id) ? 'border-navy-900 bg-navy-900 text-white' : 'border-navy-200 text-transparent hover:border-navy-400' }}" title="Select">✓</button></td>
                            <td class="px-5 py-3">
                            ...
                            <td class="px-3 py-3"><span class="rounded-full px-2 py-0.5 text-eyebrow font-bold uppercase {{ $pkgMeta[$x->package] ?? 'bg-page text-muted' }}">{{ $x->package }}</span></td>
                            ...
                            <td class="px-3 py-3">
                                <div class="flex items-center justify-end gap-1 opacity-0 transition group-hover/ex:opacity-100">
                                    <button type="button" wire:click="edit({{ $x->id }})" class="rounded-lg bg-navy-50 px-1.5 py-1 text-eyebrow font-bold text-navy-600 hover:bg-navy-100">✎</button>
                                    <button type="button" wire:click="delete({{ $x->id }})" wire:confirm="Remove {{ $x->company }}?" class="rounded-lg bg-risk/10 px-1.5 py-1 text-eyebrow font-bold text-red-700 hover:bg-risk/20">✕</button>
                                </div>
                            </td>
                        </tr>
                    @endforeach
        ...
        <div class="xl:sticky xl:top-12 xl:h-fit">
            <div class="cc-panel">
                <div class="cc-head">
                    ...
                    <span class="relative text-2xs font-bold uppercase tracking-[0.18em] text-navy-900">Exhibition Control Center</span>
                </div>
                {{-- target vs actual --}}
