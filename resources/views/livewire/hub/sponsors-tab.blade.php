...
<div>
        ...
        <div class="min-w-0">
            ...
            <div class="card mb-4 p-5">
                <div class="flex flex-wrap items-end justify-between gap-4">
                    <div>
                        <p class="text-[0.6rem] font-bold uppercase tracking-[0.14em] text-muted">Sponsorship income</p>
                    ...
                    <div class="text-right">
                        <label class="text-[0.6rem] font-bold uppercase tracking-wide text-muted">Target</label>
                ...
                @if ($target > 0)
                    <div class="mt-3">
                        <div class="mb-1 flex justify-between text-[0.6rem] font-semibold text-muted">
                        ...
                        @if ($committed < $target)
                            <p class="mt-1.5 text-[0.62rem] text-muted">{{ $fmt($target - $committed) }} left to reach target.</p>
                        @endif
                    </div>
                @endif
            </div>
                        ...
                        <tbody>
                            @foreach ($sponsors as $s)
                                ...
                                <tr wire:key="sp-{{ $s->id }}" class="group/sp border-b border-line last:border-0 hover:bg-page/30 {{ $this->isSelected($s->id) ? 'bg-navy-50/60' : '' }}">
                                    <td class="pl-4"><button type="button" wire:click="toggleSelect({{ $s->id }})" class="flex h-4 w-4 items-center justify-center rounded border text-[0.55rem] {{ $this->isSelected($s->id) ? 'border-navy-900 bg-navy-900 text-white' : 'border-navy-200 text-transparent hover:border-navy-400' }}" title="Select">✓</button></td>
                                    <td class="px-5 py-3">
                                        <p class="text-sm font-semibold text-navy-900">{{ $s->name }}</p>
                                        @if ($s->notes)<p class="truncate text-[0.62rem] text-muted">{{ $s->notes }}</p>@endif
                                    </td>
                                    <td class="px-3 py-3">@if ($s->package)<span class="rounded-full px-2 py-0.5 text-[0.56rem] font-bold uppercase tracking-wide {{ $tone($s->package) }}">{{ $s->package }}</span>@else<span class="text-muted">—</span>@endif</td>
        ...
        <div class="xl:sticky xl:top-[76px] xl:h-fit">
            <div class="card overflow-hidden">
                <div class="flex items-center justify-between border-b border-line bg-navy-900 px-4 py-3">
                    ...
                    <span class="text-[0.6rem] font-semibold text-white/60">{{ $packages->count() }}</span>
                </div>
                ...
                <div class="divide-y divide-line">
                        ...
                        <div wire:key="pkg-{{ $p->id }}" class="group/pkg px-4 py-2.5">
                            @if ($editingPackageId === $p->id)
                                <div class="space-y-2">
                                    ...
                                    <div class="flex items-center gap-1.5">
                                        <span class="text-[0.6rem] font-semibold text-muted">Max</span>
                                    ...
                                    <input type="text" wire:model="packageEditBlurb" maxlength="160" placeholder="Short tagline (optional)" class="input h-8 w-full text-[0.72rem]">
                                    <textarea wire:model="packageEditBenefits" rows="4" placeholder="Benefits — one per line&#10;e.g. Logo on main stage&#10;2 exhibition booths&#10;Speaking slot" class="input w-full py-2 text-[0.72rem] leading-snug"></textarea>
                                    <div class="flex items-center gap-2">
                                        <button type="button" wire:click="savePackage" class="rounded-lg bg-navy-900 px-3 py-1 text-[0.62rem] font-bold text-white">Save</button>
                                        <button type="button" wire:click="cancelEditPackage" class="text-[0.62rem] font-semibold text-navy-500 hover:text-navy-900">Cancel</button>
                                    </div>
                                    @error('packageEditBlurb') <p class="text-[0.6rem] font-semibold text-risk">{{ $message }}</p> @enderror
                                </div>
                            @else
                                <div class="flex items-center gap-2">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            @if ($p->slots !== null)
                                                <span class="shrink-0 rounded-full px-1.5 text-[0.5rem] font-bold uppercase tracking-wide {{ $left === 0 ? 'bg-risk/10 text-red-700' : 'bg-navy-100 text-navy-600' }}">{{ $left === 0 ? 'Sold out' : $left.' left' }}</span>
                                            @endif
                                        </div>
                                        <span class="block truncate text-[0.56rem] text-muted">@if ($p->slots !== null){{ $sold }}/{{ $p->slots }} sold · @endif{{ $bn ? $bn.' '.str('benefit')->plural($bn) : 'no benefits' }}@if ($p->blurb) · {{ $p->blurb }}@endif</span>
                                    </div>
                                    ...
                                    <span class="flex shrink-0 items-center gap-1 opacity-0 transition group-hover/pkg:opacity-100">
                                        <a href="{{ route('events.sponsorship', $event) }}?package={{ urlencode($p->name) }}" target="_blank" class="rounded bg-gold-50 px-1.5 py-0.5 text-[0.58rem] font-bold text-gold-700 hover:bg-gold-100" title="Generate this package sheet">↗</a>
                                        <button type="button" wire:click="startEditPackage({{ $p->id }})" class="rounded bg-navy-50 px-1.5 py-0.5 text-[0.58rem] font-bold text-navy-600 hover:bg-navy-100" title="Edit name, price & benefits">✎</button>
                                        <button type="button" wire:click="deletePackage({{ $p->id }})" wire:confirm="Remove the “{{ $p->name }}” package? Sold sponsors keep their amount." class="rounded bg-risk/10 px-1.5 py-0.5 text-[0.58rem] font-bold text-red-700 hover:bg-risk/20" title="Delete package">✕</button>
                                    </span>
                                </div>
                            @endif
                        </div>
                    ...
                    <div class="bg-page/30 px-4 py-3">
                        ...
                        <div class="flex items-center gap-1.5">
                            ...
                            <button type="button" wire:click="addPackage" class="rounded-lg border border-gold-300 bg-gold-50 px-2.5 py-1 text-[0.6rem] font-bold text-gold-700 hover:bg-gold-100">Add</button>
                        </div>
                        @error('newPackageName') <p class="mt-1 text-[0.6rem] font-semibold text-risk">{{ $message }}</p> @enderror
                    </div>
                </div>
                ...
                <div class="border-t border-line p-4">
                    ...
                    <a href="{{ route('events.hub', [$event, 'tab' => 'budget']) }}" class="mt-2 block text-center text-[0.62rem] font-semibold text-gold-600 hover:text-gold-700">Feeds the budget's income →</a>
                </div>
            </div>
        </div>
    </div>
    ...
    {{-- ══ sell / edit modal ══ --}}
    @if ($showForm)
        <div class="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-navy-900/40 p-4 pt-16 backdrop-blur-sm" wire:key="sponsor-modal">
            <div class="card w-full max-w-lg p-6 shadow-2xl" @click.outside="$wire.set('showForm', false)">
                <div class="mb-4 flex items-center justify-between">
                    <h3 class="pf text-base font-bold text-navy-900">{{ $editingId ? 'Edit sponsor' : 'Sell a sponsorship' }}</h3>
                ...
                <form wire:submit="save" class="grid gap-3.5 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        <label class="field-label !mb-1 !text-[0.62rem]">Sponsor name</label>
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-[0.56rem] text-muted">Picking a package fills the amount.</p>
                    </div>
                    <div>
