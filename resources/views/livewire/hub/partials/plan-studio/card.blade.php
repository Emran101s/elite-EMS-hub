...
<div data-card data-item-id="{{ $item->id }}" wire:key="card-{{ $item->id }}"
    ...
    <div class="mt-2.5 flex items-center gap-2">
        <div class="h-1.5 flex-1 overflow-hidden rounded-full bg-navy-50"><div class="h-full rounded-full transition-all" style="width: {{ max($prog, $prog > 0 ? 4 : 0) }}%; background: {{ $item->statusHex() }}"></div></div>
        ...
        @if ($item->owners->isNotEmpty())
            <span class="flex -space-x-1.5">
                @foreach ($item->owners->take(3) as $o)
                    <span class="flex h-5 w-5 items-center justify-center rounded-full bg-gradient-to-br from-navy-800 to-navy-950 text-eyebrow font-bold text-gold-300 ring-2 ring-white" title="{{ $o->name }}">{{ $ini($o->name) }}</span>
                @endforeach
                @if ($item->owners->count() > 3)<span class="flex h-5 w-5 items-center justify-center rounded-full bg-navy-100 text-eyebrow font-bold text-navy-600 ring-2 ring-white">+{{ $item->owners->count() - 3 }}</span>@endif
            </span>
        @endif
        @if ($st)
