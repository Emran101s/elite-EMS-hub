<div x-data="{ open: false }" class="relative shrink-0" data-block-drag>
    ...
    <div x-show="open" x-cloak @click.outside="open = false" x-transition
         class="absolute right-0 top-6 z-30 w-36 overflow-hidden rounded-lg border border-line bg-white py-1 shadow-overlay">
        <button type="button" wire:click="openItem({{ $item->id }})" @click="open = false"
                class="block w-full px-3 py-1.5 text-left text-micro font-semibold text-ink transition hover:bg-page">Edit details</button>
