        ...
        <div x-data="{ at: null }" class="min-w-[820px]">
            ...
            @foreach ($tracks as $t)
                ...
                <div class="flex items-center gap-2 border-b border-eo-line/70 bg-eo-workspace/60 px-4 py-1.5">
                    ...
                    <span class="text-eyebrow font-bold uppercase tracking-wide text-eo-text">{{ $t->name }}</span>
                    <span class="text-eyebrow font-semibold text-eo-muted">{{ $rows->count() }}</span>
                    @if ($t->goal)<span class="truncate text-eyebrow italic text-eo-muted">— {{ $t->goal }}</span>@endif
                ...
                @foreach ($rows as $item)
                    ...
                    <div wire:key="lir-{{ $item->id }}" class="border-b border-eo-line/50">
                        <div class="grid {{ $cols }} items-center gap-2 px-4 py-2 transition hover:bg-eo-workspace/60">
                            ...
                            <span class="flex -space-x-1.5">
                                @foreach ($item->owners->take(3) as $o)<span class="flex h-5 w-5 items-center justify-center rounded-full bg-eo-navy text-eyebrow font-bold text-white ring-2 ring-white" title="{{ $o->name }}">{{ $ini($o->name) }}</span>@endforeach
                                @if ($item->owners->isEmpty())<span class="text-eyebrow italic text-eo-muted">—</span>@endif
                            </span>
                            ...
                            <div class="flex items-center gap-1.5"><div class="h-1.5 flex-1 overflow-hidden rounded-full bg-eo-bg"><div class="h-full rounded-full" style="width: {{ max($prog, $prog > 0 ? 4 : 0) }}%; background: {{ $item->statusHex() }}"></div></div><span class="text-eyebrow font-black text-eo-muted">{{ $prog }}%</span></div>
                        </div>
                        ...
                        <div x-show="at === {{ $item->id }}" x-collapse.duration.300ms x-cloak
                            ...
                            @foreach ($item->subtasks as $sub)
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="flex-1 truncate text-micro {{ $sub->is_done ? 'text-eo-muted line-through' : 'text-eo-text' }}">{{ $sub->title }}</span>
                                    @if ($sub->owner)<span class="text-eyebrow font-semibold text-eo-muted">{{ \Illuminate\Support\Str::before($sub->owner->name, ' ') }}</span>@endif
                                    @if ($sub->due_on)<span class="text-eyebrow {{ $sub->isOverdue() ? 'font-bold text-eo-risk-ink' : 'text-eo-muted' }}">{{ $sub->due_on->format('j M') }}</span>@endif
                                </div>
                            @endforeach
                        </div>
                    </div>
