<div x-data="{ open: false }" class="relative shrink-0" data-block-drag>
    ...
    <div x-show="open" x-cloak @click.outside="open = false" x-transition
         class="absolute right-0 top-6 z-30 w-32 overflow-hidden rounded-xl border border-eo-line bg-white py-1 shadow-eo-float">
        <button type="button" wire:click="openTask({{ $item->id }})" @click="open = false" class="block w-full px-3 py-1.5 text-left text-micro font-semibold text-eo-text transition hover:bg-eo-bg">Edit details</button>
