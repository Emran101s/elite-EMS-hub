<div class="space-y-5">
    @forelse ($byModule as $slug => $rows)
        ...
        <div wire:key="tgal-{{ $slug }}">
            <div class="mb-2.5 flex items-center gap-2">
                ...
                <h3 class="text-micro font-bold uppercase tracking-wide text-eo-text">{{ $mlabel }}</h3>
                <span class="rounded-full bg-eo-bg px-1.5 text-eyebrow font-bold text-eo-muted">{{ $rows->count() }}</span>
                <button type="button" wire:click="addTask('{{ $slug }}')" class="ml-auto text-eyebrow font-bold text-eo-muted transition hover:text-eo-teal-ink">＋ Task</button>
            </div>
    ...
    @if ($unmoduled->isNotEmpty())
        <div wire:key="tgal-none">
            <h3 class="mb-2.5 text-micro font-bold uppercase tracking-wide text-eo-muted">No module</h3>
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
