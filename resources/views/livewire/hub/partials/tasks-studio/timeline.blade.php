    ...
    <div class="overflow-x-auto">
        <div class="min-w-[840px]">
            <div class="flex border-b border-line bg-white">
                <div class="w-[210px] shrink-0 px-4 py-2.5 text-eyebrow font-bold uppercase tracking-wide text-navy-400">Task</div>
            ...
            <div class="relative">
                ...
                @forelse ($byModule as $slug => $rows)
                    ...
                    <div class="flex items-center gap-2 border-b border-line/70 bg-navy-50/30 px-3 py-1">
                        ...
                        <span class="text-eyebrow font-bold uppercase tracking-wide text-navy-600">{{ $mlabel }}</span>
                    </div>
                    ...
                    @foreach ($rows as $item)
                        ...
                        <div class="group flex items-stretch border-b border-line/50 transition hover:bg-navy-50/30">
                            ...
                            <div class="relative min-w-0 flex-1" style="min-height: 38px">
                                ...
                                @else
                                    <button type="button" wire:click="openTask({{ $item->id }})" class="absolute left-2 top-1/2 -translate-y-1/2 text-eyebrow italic text-navy-300 hover:text-gold-600">set dates…</button>
                                @endif
                            </div>
                        </div>
                    @endforeach
                @empty
                    <div class="px-6 py-12 text-center text-xs text-muted">No tasks to place yet.</div>
                @endforelse

                @if ($unmoduled->isNotEmpty())
                    <div class="flex items-center gap-2 border-b border-line/70 bg-navy-50/30 px-3 py-1"><span class="h-2.5 w-2.5 rounded-[3px] bg-navy-300"></span><span class="text-eyebrow font-bold uppercase tracking-wide text-navy-500">No module</span></div>
                    @foreach ($unmoduled as $item)
                        @php $g = $geo[$item->id] ?? null; $prog = $item->progress(); $hex = $item->stageHex(); @endphp
                        <div class="flex items-stretch border-b border-line/50">
                            <div class="flex w-[210px] shrink-0 items-center gap-2 px-3 py-1.5"><span class="h-2 w-2 rounded-full" style="background: {{ $item->priorityHex() }}"></span><button type="button" wire:click="openTask({{ $item->id }})" class="truncate text-left text-xs font-semibold text-navy-800">{{ $item->title ?: 'Untitled task' }}</button></div>
                            <div class="relative min-w-0 flex-1" style="min-height: 38px">
                                @if ($g)<button type="button" wire:click="openTask({{ $item->id }})" class="absolute top-1/2 flex h-[24px] -translate-y-1/2 items-center gap-1.5 overflow-hidden rounded-lg px-2 text-left" style="left: {{ $g['left'] }}%; width: {{ $g['width'] }}%; min-width: 92px; background: {{ $hex }}1F; box-shadow: inset 0 0 0 1px {{ $hex }}55"><span class="text-eyebrow font-bold text-navy-800">{{ $prog }}%</span></button>@else<button type="button" wire:click="openTask({{ $item->id }})" class="absolute left-2 top-1/2 -translate-y-1/2 text-eyebrow italic text-navy-300">set dates…</button>@endif
                            </div>
                        </div>
                    @endforeach
                @endif
