    ...
    <div class="overflow-x-auto">
        <div class="min-w-[840px]">
            <div class="flex border-b border-line bg-white">
                <div class="w-[210px] shrink-0 px-4 py-2.5 text-eyebrow font-bold uppercase tracking-wide text-muted">Task</div>
                <div class="relative h-9 flex-1">
                    ...
                    @endforeach
                    @if ($axis['todayIn'])<span class="absolute top-1 z-20 -translate-x-1/2 rounded-full bg-navy-900 px-2 py-0.5 text-eyebrow font-bold tracking-wide text-white shadow" style="left: {{ $axis['todayLeft'] }}%">TODAY</span>@endif
                </div>
            </div>
            ...
            <div class="relative">
                ...
                @forelse ($byModule as $slug => $rows)
                    ...
                    @foreach ($rows as $item)
                        ...
                        <div class="group flex items-stretch border-b border-line/50 transition hover:bg-page">
                            <div class="flex w-[210px] shrink-0 items-center gap-2 px-3 py-1.5">
                                ...
                                @if ($item->isSigned())<span class="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-gold-500 text-eyebrow font-black text-navy-900">✓</span>@endif
                            ...
                            <div class="relative min-w-0 flex-1" style="min-height: 38px">
                                @foreach ($axis['ticks'] as $t)<span class="pointer-events-none absolute inset-y-0 w-px bg-line/30" style="left: {{ $t['left'] }}%"></span>@endforeach
                                @if ($g)
                                ...
                                @else
                                    <button type="button" wire:click="openTask({{ $item->id }})" class="absolute left-2 top-1/2 -translate-y-1/2 text-eyebrow italic text-muted hover:text-gold-700">set dates…</button>
                                @endif
                            </div>
                        </div>
                    @endforeach
                ...
                @if ($unmoduled->isNotEmpty())
                    <div class="flex items-center gap-2 border-b border-line/70 bg-page px-3 py-1"><span class="h-2.5 w-2.5 rounded-[3px] bg-muted"></span><span class="text-eyebrow font-bold uppercase tracking-wide text-muted">No module</span></div>
                    @foreach ($unmoduled as $item)
                        @php $g = $geo[$item->id] ?? null; $prog = $item->progress(); $hex = $item->stageHex(); @endphp
                        <div class="flex items-stretch border-b border-line/50">
                            <div class="flex w-[210px] shrink-0 items-center gap-2 px-3 py-1.5"><span class="h-2 w-2 rounded-full" style="background: {{ $item->priorityHex() }}"></span><button type="button" wire:click="openTask({{ $item->id }})" class="truncate text-left text-xs font-semibold text-ink">{{ $item->title ?: 'Untitled task' }}</button></div>
                            <div class="relative min-w-0 flex-1" style="min-height: 38px">
                                @if ($g)<button type="button" wire:click="openTask({{ $item->id }})" class="absolute top-1/2 flex h-[24px] -translate-y-1/2 items-center gap-1.5 overflow-hidden rounded-lg px-2 text-left" style="left: {{ $g['left'] }}%; width: {{ $g['width'] }}%; min-width: 92px; background: {{ $hex }}1F; box-shadow: inset 0 0 0 1px {{ $hex }}55"><span class="text-eyebrow font-bold text-ink">{{ $prog }}%</span></button>@else<button type="button" wire:click="openTask({{ $item->id }})" class="absolute left-2 top-1/2 -translate-y-1/2 text-eyebrow italic text-muted">set dates…</button>@endif
                            </div>
                        </div>
                    @endforeach
                @endif
