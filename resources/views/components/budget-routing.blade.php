...
<div class="mt-3 border-t border-line pt-3">
    <p class="text-eyebrow font-bold uppercase tracking-[0.14em] text-muted">Costs go to</p>

    <details class="relative mt-1.5" data-menu>
        <summary class="flex cursor-pointer list-none items-center gap-2 rounded-xl border border-line bg-white px-3 py-2 text-[11.5px] font-semibold text-ink transition hover:border-gold-400 [&::-webkit-details-marker]:hidden">
            ...
            @foreach ($routing['categories'] as $name)
                <button type="button" wire:click="routeCostsTo(@js($name))" wire:key="cat-{{ $loop->index }}"
                        @class([
                            ...
                            'bg-navy-900 text-white' => $name === $routing['current'],
                            'text-ink hover:bg-page' => $name !== $routing['current'],
                        ])>
