...
<div class="mt-3 border-t border-eo-line pt-3">
    <p class="text-eyebrow font-bold uppercase tracking-[0.14em] text-eo-muted">Costs go to</p>

    <details class="relative mt-1.5" data-menu>
        <summary class="flex cursor-pointer list-none items-center gap-2 rounded-xl border border-eo-line bg-white px-3 py-2 text-[11.5px] font-semibold text-eo-text transition hover:border-eo-teal [&::-webkit-details-marker]:hidden">
            <span class="min-w-0 flex-1 truncate">{{ $routing['current'] }}</span>
            <span class="shrink-0 text-eo-muted">▾</span>
        </summary>

        <div class="absolute z-30 mt-1 max-h-64 w-64 overflow-y-auto rounded-2xl border border-eo-line bg-white p-1.5 shadow-xl">
            @foreach ($routing['categories'] as $name)
                <button type="button" wire:click="routeCostsTo(@js($name))" wire:key="cat-{{ $loop->index }}"
                        @class([
                            ...
                            'bg-eo-navy text-white' => $name === $routing['current'],
                            'text-eo-text hover:bg-eo-workspace' => $name !== $routing['current'],
                        ])>
                    <span class="min-w-0 flex-1 truncate">{{ $name }}</span>
                    @if ($name === $routing['current'])<span class="shrink-0 text-eo-teal-lit">✓</span>@endif
                </button>
            @endforeach
        </div>
    </details>

    <p class="mt-1.5 text-[10.5px] leading-snug text-eo-muted">
