...
<span {{ $attributes->merge(['class' => 'inline-flex min-w-0 items-baseline gap-1.5']) }}>
    <span @class(['min-w-0 truncate', 'italic font-medium text-muted' => $name === '', 'line-through text-muted' => $muted])>
        {{ $name !== '' ? $name : $fallback }}
    </span>

    @if ($missing !== [])
        <span class="mt-px inline-block h-1.5 w-1.5 shrink-0 rounded-full bg-warning"
