...
<aside class="shell-panel relative hidden w-[288px] shrink-0 flex-col overflow-hidden xl:flex"
    ...
    <span aria-hidden="true" class="pointer-events-none absolute inset-0 overflow-hidden">
        <span class="absolute -left-28 top-20 aspect-square w-[340px] rounded-full border border-shell-gold/[0.06]"></span>
        <span class="absolute -right-40 bottom-40 aspect-square w-[420px] rounded-full border border-shell-cyan/[0.05]"></span>
        <span class="absolute -right-28 bottom-56 aspect-square w-[300px] rounded-full border border-shell-gold/[0.05]"></span>
        @foreach ([[14, 30], [78, 18], [40, 66], [88, 52], [24, 84], [62, 92]] as [$x, $y])
            <span class="absolute h-[2px] w-[2px] rounded-full bg-shell-gold-lit/25" style="left: {{ $x }}%; top: {{ $y }}%"></span>
        @endforeach
    </span>
    ...
    <div class="relative shrink-0 px-4 pb-2.5 pt-4">
        <details class="group/user relative" data-menu>
            <summary class="shell-glass shell-glass-hover flex cursor-pointer list-none items-center gap-3 rounded-[20px] px-3.5 py-3 text-left transition duration-200 ease-out hover:-translate-y-px [&::-webkit-details-marker]:hidden">
                <span class="relative shrink-0">
                    <x-user-avatar :user="$user" size="h-11 w-11" />
                    <span aria-hidden="true" class="absolute inset-0 rounded-full ring-[1.5px] ring-shell-gold/50"></span>
                </span>
                ...
                <span class="min-w-0 flex-1">
                    <span class="flex items-center gap-1.5">
                        ...
                        <svg viewBox="0 0 24 24" class="h-4 w-4 shrink-0 text-shell-gold-lit" fill="currentColor" aria-hidden="true">
                            <path d="M3 8l4 3 5-6 5 6 4-3v9a1 1 0 01-1 1H4a1 1 0 01-1-1V8z"/>
                        </svg>
                    </span>
                    <span class="block truncate text-[11.5px] font-semibold text-shell-gold/90">{{ $user?->title ?: 'Founder & CEO' }}</span>
                    <span class="block truncate text-[10.5px] text-shell-dim">{{ $user?->email }}</span>
                </span>

                <x-icon name="chevron" class="h-4 w-4 shrink-0 text-white/35 transition duration-200 group-open/user:rotate-180" />
            </summary>

            <div class="shell-glass absolute inset-x-0 z-40 mt-1.5 overflow-hidden rounded-2xl py-1">
    ...
    @if ($activeEvents > 0 && Route::has('events.index'))
        <div class="relative shrink-0 px-4 pb-3">
            <a href="{{ route('events.index') }}"
               class="shell-glass shell-glass-hover flex items-center gap-2.5 rounded-[16px] px-3.5 py-2.5 transition duration-200 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-shell-gold/60">
                <span class="shell-pulse h-2.5 w-2.5 shrink-0 rounded-full bg-shell-online"></span>
    ...
    <nav class="scrollbar-none relative min-h-0 flex-1 overflow-y-auto px-4 pb-4" aria-label="Sections">
        @foreach ($sections->concat($extra) as $section)
            ...
            <div class="mb-1 mt-3 flex items-center gap-3 px-2 first:mt-0.5">
                <span class="shrink-0 text-[10px] font-bold uppercase tracking-[0.24em] text-shell-gold">{{ $section['label'] }}</span>
                <span aria-hidden="true" class="h-px flex-1 bg-gradient-to-r from-shell-gold/30 via-shell-gold/12 to-transparent"></span>
                <span aria-hidden="true" class="h-[3px] w-[3px] shrink-0 rounded-full bg-shell-gold/70"></span>
            </div>
            ...
            @foreach ($section['items'] as $item)
                ...
                <a href="{{ $item['href'] }}"
                    ...
                    <x-icon :name="$item['icon']" @class([
                        ...
                        'text-shell-gold-lit' => $item['active'],
                    ...
                    @elseif ($creates)
                        <span aria-hidden="true" class="shrink-0 text-[17px] leading-none text-white/30 transition duration-200 group-hover:text-shell-gold-lit">＋</span>
                    @elseif ($item['count'])
    ...
    <div class="relative shrink-0 px-4 pb-4 pt-1">
        <div class="shell-glass relative overflow-hidden rounded-[20px] p-1.5">
            ...
            <a href="{{ route('settings.index') }}"
               class="group relative flex h-10 items-center gap-3 rounded-[14px] px-3 transition duration-200 ease-out hover:bg-white/[0.07] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-shell-gold/60">
            ...
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit"
                        class="group relative flex h-10 w-full items-center gap-3 rounded-[14px] px-3 transition duration-200 ease-out hover:bg-white/[0.07] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-shell-gold/60">
