...
<div class="flex flex-wrap items-center gap-3">
    <div class="inline-flex items-center gap-1 rounded-xl border border-line bg-white p-1 shadow-raise">
        @foreach ($views as $key => [$label, $icon])
            <button type="button" wire:click="setView('{{ $key }}')"
                    @class([
                        'relative flex items-center gap-2 rounded-lg px-3.5 py-2 text-[12.5px] font-bold transition',
