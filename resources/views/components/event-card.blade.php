...
@php
    ...
    $stage = Tone::forEventStage($event->stage);
...
<div wire:click="select({{ $event->id }})"
     @class([
         ...
         'border-gold-400 shadow-[var(--lift-2)] ring-1 ring-gold-300' => $selected,
         'border-line shadow-[var(--lift-1)] hover:-translate-y-1 hover:border-gold-300 hover:shadow-[var(--lift-2)]' => ! $selected,
     ])>

    {{-- stage edge --}}
    <span class="absolute inset-x-0 top-0 z-20 h-1" style="background: {{ $stage->lit() }}" title="{{ str($event->stage)->replace('_', ' ')->title() }}"></span>
    ...
    <div class="relative h-[124px] shrink-0 overflow-hidden bg-gradient-to-br from-navy-800 to-navy-950">
        <div class="pointer-events-none absolute -right-7 -top-9 h-28 w-28 rounded-full bg-[radial-gradient(circle,var(--gold-glow),transparent_70%)]"></div>
    ...
    <div class="flex min-h-0 flex-1 flex-col p-3.5">
        ...
        <div class="mt-auto">
            <div class="mb-1 flex items-baseline justify-between">
                ...
                <span class="text-[0.72rem] font-black" style="color: {{ $usage->var() }}">{{ $used !== null ? $used.'%' : '—' }}</span>
            </div>
            <div class="h-1.5 overflow-hidden rounded-full bg-navy-50">
                <div class="h-full rounded-full transition-all" style="width: {{ min($used ?? 0, 100) }}%; background: {{ $usage->lit() }}"></div>
            </div>
