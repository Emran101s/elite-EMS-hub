...
@if ($count > 0)
    <div class="bulkbar-in fixed bottom-6 left-1/2 z-50 -translate-x-1/2">
        <div class="flex items-center gap-1 rounded-full border border-eo-line bg-white/95 py-1.5 pl-2 pr-1.5 shadow-eo-float backdrop-blur">
            <span class="flex items-center gap-2 pl-1.5 pr-1 text-xs font-semibold text-eo-text">
                <span class="flex h-5 min-w-[1.25rem] items-center justify-center rounded-full bg-eo-navy px-1.5 text-[0.62rem] font-bold text-white">{{ $count }}</span>
                {{ \Illuminate\Support\Str::plural($noun, $count) }} selected
            </span>
            <span class="mx-0.5 h-5 w-px bg-eo-line"></span>
            <x-confirm title="Delete the {{ $count }} selected {{ \Illuminate\Support\Str::plural($noun, $count) }}?"
                       confirm="Delete" run="$wire.deleteSelected()"
                       class="flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold text-eo-risk-ink transition hover:bg-eo-risk-soft">
            ...
            <button type="button" wire:click="clearSelection" title="Clear selection"
                    class="flex h-8 w-8 items-center justify-center rounded-full text-eo-muted transition hover:bg-eo-bg hover:text-eo-text">
